...
*/
uint16_t Mid_CalculateRunPRESS(uint16_t set_press,eBreathe_Stage now_stage){
	...
	if(now_stage == None){
		...
		if(flag_inscount == RESET){
			ins_count = ins_count>=50 ? 50 : ins_count+1;
			if(ins_count == 50) flag_inscount = SET;
		}
		...
		epr_press = set_press - (Set_Param.epr*50);
		epr_press = epr_press<MACHINE_MINPRESS ? MACHINE_MINPRESS : epr_press;
		return_press = epr_press;
	}
	else if(now_stage == Ex_End){
		return_press = epr_press + (5 * ((++ex_count)/2)); 	//Max time = 2s.
