	...
	while(1){
		...
		if(Machine_DelayIP <= run_stage && run_stage <= Machine_Run){
			...
			/* Machine is running. */
				...
				MoveRight_Range(Run_Param.flow_mean_5,5,flow_mean5);
				Mid_Judge_BreatheStage_New(flow_buff,Run_Param.flow_mean,Run_Param.flow_mean_5,&Run_Param.breathe_stage);
				Mid_CalculateRunPRESS(&Run_Param.now_run_p,Run_Param.now_set_p,Run_Param.breathe_stage);
			}
		}
		else{
			flow_buff_count = 0;
		...
		/* Judge the breathing stage and update the control pressure. */
		// if(run_stage == Machine_DelayIP || run_stage == Machine_NoDelayIP || run_stage == Machine_Run){
