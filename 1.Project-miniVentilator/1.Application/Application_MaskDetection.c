	...
	while(1){
		...
		switch(testmask_stge){
			...
			case Test_Run:
				...
				if(Run_Param.leak_mean > MAX_MASKFLOWMEAN && Machine_State.flag_ajustmask == RESET)
					Machine_State.flag_ajustmask = SET;
				else if(Run_Param.leak_mean < MIN_MASKFLOWMEAN && Machine_State.flag_ajustmask == SET)
					Machine_State.flag_ajustmask = RESET;
				break;
			default:break;
		}
