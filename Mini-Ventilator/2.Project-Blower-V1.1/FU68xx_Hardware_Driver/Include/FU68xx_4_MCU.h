...
/******************************************************************************///RTC
 #define RTC_TM							*(_IO uint16 FT_XDATA *)0x402c
 #define RTC_STA						*(_IO  uint8 FT_XDATA *)0x402e
 #define RTC_EN									  0x80							//				 rw-- RTC使能
 #define RTC_IF									  0x40							//				 rw-- RTC中断标志
 #define ISOSCEN								  0x10							//				 rw-- 内部慢时钟使能
/******************************************************************************///CLOCK_CAL
 #define CAL_CR0						*(_IO  uint8 FT_XDATA *)0x4044
 #define CAL_STA								  0x80							//				 w-- CLOCK_CAL使能,	r--	0-->Done 1-->ING
 #define CAL_CR1						*(_IO  uint8 FT_XDATA *)0x4045
/******************************************************************************///IRQ
 //	IRQ	Channel
 //	LVW/TSD			interrupt 0
 //	INT0			interrupt 1
...
//	SBIT(PCMP30,	0xd8, 0);
/******************************************************************************///I2C
 #define I2C_CR							*(_IO  uint8 FT_XDATA *)0x4028			   // RV:  00H	I2C控制寄存器
 ...
 #define I2C_SR							*(_IO  uint8 FT_XDATA *)0x402b			   // RV:  00H	I2C状态寄存器
 ...
 #define I2C_ID							*(_IO  uint8 FT_XDATA *)0x4029			   // RV:  00H	本机2C地址
 #define GC										  0x01							//				 广播模式

 #define I2C_DR							*(_IO  uint8 FT_XDATA *)0x402a			   // RV:  00H	I2C数据寄存器
/******************************************************************************///SPI
 #define SPI_CR0						*(_IO  uint8 FT_XDATA *)0x4030			   // RV:  0bH	SPI控制寄存器1
 ...
 #define SPI_CR1						*(_IO  uint8 FT_XDATA *)0x4031			   // RV:  02H	SPI控制寄存器2
 ...
 #define SPI_CLK						*(_IO  uint8 FT_XDATA *)0x4032			   // RV:  00H	SPI时钟配置寄存器
 #define SPI_DR							*(_IO  uint8 FT_XDATA *)0x4033			   // RV:  00H	SPI数据寄存器
/******************************************************************************///UART
...
// SFR(UT2_CR, 0x8a);
 ...
 #define UT2_BAUD						*(_IO uint16 FT_XDATA *)0x4042			   // RV:009bH	UART2波特率控制寄存器
 ...
 //sfr	   UT2_DR						  =	0x89;								  // RV:  00H  UART2数据寄存器
  SFR(UT2_DR, 0x89);
/******************************************************************************///LIN
 #define LIN_CR							*(_IO  uint8 FT_XDATA *)0x40e0			   // RV:0000H	LIN控制寄存器
 ...
 #define LIN_SR							*(_IO  uint8 FT_XDATA *)0x40e1			   // RV:0000H	LIN状态寄存器
 ...
 #define LIN_CSR						*(_IO  uint8 FT_XDATA *)0x40e2			   // RV:0000H	LIN状态控制寄存器
 ...
 #define LIN_ID							*(_I   uint8 FT_XDATA *)0x40e3			   // RV:0000H	LIN收到的ID的寄存器
 #define LIN_SIZE						*(_IO  uint8 FT_XDATA *)0x40e4			   // RV:0000H	LIN数据长度寄存器
 #define LIN_BAUD						*(_I  uint16 FT_XDATA *)0x40e6			   // RV:04afH	LIN波特率寄存器
/******************************************************************************///MDU
 ...
 //sfr	   MDU_D						  =	0xcb;								  // RV:  00H  MDU数据寄存器D
  ...
  SFR(MDU_D, 0xcb);
/******************************************************************************///PI
 #define PI0_KP							*(_IO  uint16 FT_XDATA *)0x02e0			   // RV:0000H	PI1上二个输入误差
 #define PI0_KI							*(_IO  uint16 FT_XDATA *)0x02e2			   // RV:0000H	PI1的微分参数
 #define PI0_UKMAX						*(_IO  uint16 FT_XDATA *)0x02e4			   // RV:0000H	PI0的比例参数
 #define PI0_UKMIN						*(_IO  uint16 FT_XDATA *)0x02e6			   // RV:0000H	PI0的积分参数
 #define PI0_EK1						*(_IO  uint16 FT_XDATA *)0x02e8			   // RV:0000H	PI0的输出允许的最大值
 #define PI0_EK							*(_IO  uint16 FT_XDATA *)0x02ea			   // RV:0000H	PI0的输出允许的最小值
 #define PI0_UKH						*(_IO  uint16 FT_XDATA *)0x02ec			   // RV:0000H	PI0的上一个输入误差
 #define PI0_UKL						*(_IO  uint16 FT_XDATA *)0x02ee			   // RV:0000H	PI0的误差输入

 #define PI1_KP							*(_IO  uint16 FT_XDATA *)0x02d0			   // RV:0000H	PI1的比例参数
 #define PI1_KI							*(_IO  uint16 FT_XDATA *)0x02d2			   // RV:0000H	PI1的积分参数
 #define PI1_UKMAX						*(_IO  uint16 FT_XDATA *)0x02d4			   // RV:0000H	PI1的输出允许的最大值
 #define PI1_UKMIN						*(_IO  uint16 FT_XDATA *)0x02d6			   // RV:0000H	PI1的输出允许的最小值
 #define PI1_EK1						*(_IO  uint16 FT_XDATA *)0x02d8			   // RV:0000H	PI1的上一个输入误差
 #define PI1_EK							*(_IO  uint16 FT_XDATA *)0x02da			   // RV:0000H	PI1的误差输入
 #define PI1_UKH						*(_IO  uint16 FT_XDATA *)0x02dc			   // RV:0000H	PI1的输出结果高16位
 #define PI1_UKL						*(_IO  uint16 FT_XDATA *)0x02de			   // RV:0000H	PI1的输出结果低16位

 #define PI2_KP							*(_IO  uint16 FT_XDATA *)0x02bc			   // RV:0000H	PI2的比例参数
 #define PI2_KI							*(_IO  uint16 FT_XDATA *)0x02be			   // RV:0000H	PI2的积分参数
 #define PI2_UKMAX						*(_IO  uint16 FT_XDATA *)0x02c0			   // RV:0000H	PI2的输出允许的最大值
 #define PI2_UKMIN						*(_IO  uint16 FT_XDATA *)0x02c2			   // RV:0000H	PI2的输出允许的最小值
 #define PI2_EK1						*(_IO  uint16 FT_XDATA *)0x02c4			   // RV:0000H	PI2的上一个输入误差
 #define PI2_EK							*(_IO  uint16 FT_XDATA *)0x02c6			   // RV:0000H	PI2的误差输入
 #define PI2_UKH						*(_IO  uint16 FT_XDATA *)0x02c8			   // RV:0000H	PI2的输出结果高16位
 #define PI2_UKL						*(_IO  uint16 FT_XDATA *)0x02ca			   // RV:0000H	PI2的输出结果低16位
 #define PI2_KD							*(_IO  uint16 FT_XDATA *)0x02cc			   // RV:0000H	PI2上二个输入误差
 #define PI2_EK2						*(_IO  uint16 FT_XDATA *)0x02ce			   // RV:0000H	PI2的微分参数

 #define PI3_KP							*(_IO  uint16 FT_XDATA *)0x02a8			   // RV:0000H	PI3的比例参数
 #define PI3_KI							*(_IO  uint16 FT_XDATA *)0x02aa			   // RV:0000H	PI3的积分参数
 #define PI3_UKMAX						*(_IO  uint16 FT_XDATA *)0x02ac			   // RV:0000H	PI3的输出允许的最大值
 #define PI3_UKMIN						*(_IO  uint16 FT_XDATA *)0x02ae			   // RV:0000H	PI3的输出允许的最小值
 #define PI3_EK1						*(_IO  uint16 FT_XDATA *)0x02b0			   // RV:0000H	PI3的上一个输入误差
 #define PI3_EK							*(_IO  uint16 FT_XDATA *)0x02b2			   // RV:0000H	PI3的误差输入
 #define PI3_UKH						*(_IO  uint16 FT_XDATA *)0x02b4			   // RV:0000H	PI3的输出结果高16位
 #define PI3_UKL						*(_IO  uint16 FT_XDATA *)0x02b6			   // RV:0000H	PI3的输出结果低16位
 #define PI3_KD							*(_IO  uint16 FT_XDATA *)0x02b8			   // RV:0000H	PI3上二个输入误差
 #define PI3_EK2						*(_IO  uint16 FT_XDATA *)0x02ba			   // RV:0000H	PI3的微分参数
...
/******************************************************************************///FOC
 #define FOC_CR0						*(_IO  uint8 FT_XDATA *)0x409f			   // RV:  00H	FOC控制寄存器0
 ...
 #define FOC_CR1						*(_IO  uint8 FT_XDATA *)0x40a0			   // RV:  00H	FOC控制寄存器1
 ...
 #define FOC_CR2						*(_IO  uint8 FT_XDATA *)0x40a1			   // RV:  00H	FOC控制寄存器2
 ...
 #define FOC_EKP						*(_IO  int16 FT_XDATA *)0x4074			   // RV:0000H	估算器的PI的P参数
 #define FOC_EKI						*(_IO  int16 FT_XDATA *)0x4076			   // RV:0000H	估算器的PI的I参数
 #define FOC_KSLIDE						*(_IO  int16 FT_XDATA *)0x4078			   // RV:0000H	估算器SMO模式下的KSLIDE	 或	 PLL模式下的KP
 #define FOC_EKLPFMIN					*(_IO  int16 FT_XDATA *)0x407a			   // RV:0000H	估算器SMO模式下BMF的LPF系数最小值  或  PLL模式下的KI
 #define FOC_EBMFK						*(_IO  int16 FT_XDATA *)0x407c			   // RV:0000H	估算器反电动势的LPF系数
 #define FOC_OMEKLPF					*(_IO  int16 FT_XDATA *)0x407e			   // RV:0000H	估算器速度计算的LPF系数
 #define FOC_FBASE						*(_IO  int16 FT_XDATA *)0x4080			   // RV:0000H	估算器角度增量(OMEGA)的系数
 #define FOC_EFREQACC					*(_IO uint16 FT_XDATA *)0x4082			   // RV:0000H	估算器在强制角度模式下的OMEGA增量
 #define FOC_EFREQMIN					*(_IO  int16 FT_XDATA *)0x4084			   // RV:0000H	估算器在强制角度模式下的OMEGA最小值
 #define FOC_EFREQHOLD					*(_IO  int16 FT_XDATA *)0x4086			   // RV:0000H	估算器在强制角度模式下的OMEGA保持值
 #define FOC_EFREQMAX					*(_IO  uint8 FT_XDATA *)0x406f			   // RV:  00H	估算器的强制角度模式下的OMEGA最大值
 #define FOC_EK3						*(_IO  int16 FT_XDATA *)0x4088			   // RV:0000H	估算器估算电流的系数3
 #define FOC_EK4						*(_IO  int16 FT_XDATA *)0x408a			   // RV:0000H	估算器估算电流的系数4
 #define FOC_EK1						*(_IO  int16 FT_XDATA *)0x408c			   // RV:0000H	估算器估算电流的系数1
 #define FOC_EK2						*(_IO  int16 FT_XDATA *)0x408e			   // RV:0000H	估算器估算电流的系数2
 #define FOC_IDREF						*(_IO  int16 FT_XDATA *)0x4090			   // RV:0000H	D轴电流参考值
 #define FOC_IQREF						*(_IO  int16 FT_XDATA *)0x4092			   // RV:0000H	Q轴电流参考值
 #define FOC_DQKP						*(_IO  int16 FT_XDATA *)0x4094			   // RV:0000H	DQ轴PI的比例参数
 #define FOC_DQKI						*(_IO  int16 FT_XDATA *)0x4096			   // RV:0000H	DQ轴PI的积分参数
 #define FOC__UDCFLT					*(_I   int16 FT_XDATA *)0x4098			   // RV:0000H	滤波后的母线电压
 #define FOC_TSMIN						*(_IO  uint8 FT_XDATA *)0x40a2			   // RV:  00H	单电阻模式下的ADC采样窗口  或  双三电阻模式下死区补偿
 #define FOC_TGLI						*(_IO  uint8 FT_XDATA *)0x40a3			   // RV:  00H	上桥导通窄脉冲消除
 #define FOC_TBLO						*(_IO  uint8 FT_XDATA *)0x40a4			   // RV:  00H	三电阻电流采样的屏蔽时间
 #define FOC_TRGDLY						*(_IO	int8 FT_XDATA *)0x40a5			   // RV:  00H	单电阻模式下ADC采样触发延迟	 或	 双三电阻模式下电流采样时机
 #define FOC_CSO						*(_IO  int16 FT_XDATA *)0x40a6			   // RV:4040H	电流采样基准值
 #define FOC__RTHESTEP					*(_IO  int16 FT_XDATA *)0x40a8			   // RV:0000H	爬坡速度
 #define FOC_RTHEACC					*(_O   int16 FT_XDATA *)0x40aa			   // RV:0000H	爬坡加速度
 #define FOC__EOMELPF					*(_I   int16 FT_XDATA *)0x40aa			   // RV:0000H	滤波后速度
 #define FOC__RTHECNT					*(_IO  uint8 FT_XDATA *)0x40ac			   // RV:  00H	爬坡次数
 #define FOC__THECOR					*(_IO  uint8 FT_XDATA *)0x40ad			   // RV:  01H	爬坡模式切换到估算模式的角度修正值
 #define FOC__EMF						*(_I   int16 FT_XDATA *)0x40ae			   // RV:0000H	估算器估算的反电动势EMF
 #define FOC__THECOMP					*(_O   int16 FT_XDATA *)0x40ae			   // RV:0000H	角度补偿值，用于补偿估算器估算出的角度
 #define FOC_DMAX						*(_IO  int16 FT_XDATA *)0x40b0			   // RV:0000H	D轴PI控制器的输出上限
 #define FOC_DMIN						*(_IO  int16 FT_XDATA *)0x40b2			   // RV:0000H	D轴PI控制器的输出下限
 #define FOC_QMAX						*(_IO  int16 FT_XDATA *)0x40b4			   // RV:0000H	Q轴PI控制器的输出上限
 #define FOC_QMIN						*(_IO  int16 FT_XDATA *)0x40b6			   // RV:0000H	Q轴PI控制器的输出下限
 #define FOC__UD						*(_IO  int16 FT_XDATA *)0x40b8			   // RV:0000H	D轴电压输出值
 #define FOC__UQ						*(_IO  int16 FT_XDATA *)0x40ba			   // RV:0000H	Q轴电压输出值
 #define FOC__ID						*(_I   int16 FT_XDATA *)0x40bc			   // RV:0000H	D轴反馈电流值
 #define FOC__IQ						*(_I   int16 FT_XDATA *)0x40be			   // RV:0000H	Q轴反馈电流值
 #define FOC__IBET						*(_I   int16 FT_XDATA *)0x40c0			   // RV:0000H	β轴采样电流值
 #define FOC__VBET						*(_I   int16 FT_XDATA *)0x40c2			   // RV:0000H	β轴电压输出值
 #define FOC_UDCPS						*(_O   int16 FT_XDATA *)0x40c2			   // RV:0000H	D轴的电压前馈补偿
 #define FOC__VALP						*(_I   int16 FT_XDATA *)0x40c4			   // RV:0000H	α轴电压输出值
 #define FOC_UQCPS						*(_O   int16 FT_XDATA *)0x40c4			   // RV:0000H	Q轴的电压前馈补偿
 #define FOC__IC						*(_I   int16 FT_XDATA *)0x40c6			   // RV:0000H	C相电流
 #define FOC__IB						*(_I   int16 FT_XDATA *)0x40c8			   // RV:0000H	B相电流
 #define FOC__IA						*(_I   int16 FT_XDATA *)0x40ca			   // RV:0000H	A相电流
 #define FOC__THETA						*(_IO  int16 FT_XDATA *)0x40cc			   // RV:0000H	电机电角度
 #define FOC__ETHETA					*(_IO  int16 FT_XDATA *)0x40ce			   // RV:0000H	估算器估算的角度
 #define FOC__EALP						*(_IO  int16 FT_XDATA *)0x40d0			   // RV:0000H	估算器估算反电动势的α轴值
 #define FOC__EBET						*(_IO  int16 FT_XDATA *)0x40d2			   // RV:0000H	估算器估算反电动势的β轴值
 #define FOC__EOME						*(_IO  int16 FT_XDATA *)0x40d4			   // RV:0000H	估算器估算的速度
 #define FOC__UQEX						*(_I   int16 FT_XDATA *)0x40d6			   // RV:0000H	Q轴的电压溢出值，用于弱磁控制
 #define FOC_KFG						*(_O  uint16 FT_XDATA *)0x40d6			   // RV:0000H	FG计算系数
 #define FOC__POW						*(_I   int16 FT_XDATA *)0x40d8			   // RV:0000H	电机当前运行功率
 #define FOC_EOMEKLPF					*(_O   uint8 FT_XDATA *)0x40d8			   // RV:0000H	EOMELPF的滤波系数
 #define FOC__IAMAX						*(_I   int16 FT_XDATA *)0x40da			   // RV:0000H	A相电流最大值
 #define FOC__IBMAX						*(_I   int16 FT_XDATA *)0x40dc			   // RV:0000H	B相电流最大值
 #define FOC__ICMAX						*(_I   int16 FT_XDATA *)0x40de			   // RV:0000H	C相电流最大值
/******************************************************************************///TIM1
 #define TIM1_CR0						*(_IO  uint8 FT_XDATA *)0x4068			   // RV:  00H	TIMER1控制寄存器0
 ...
 #define TIM1_CR1						*(_IO  uint8 FT_XDATA *)0x4069			   // RV:  00H	TIMER1控制寄存器1
 #define T1BAPE									  0x80							//				 rw-- TIM1__BARR自动装载使能

 #define TIM1_CR2						*(_IO  uint8 FT_XDATA *)0x406a			   // RV:  00H	TIMER1控制寄存器2
 #define T1BRS									  0x80							//				 rw-- 基本定时器复位源选择

 #define TIM1_CR3						*(_IO  uint8 FT_XDATA *)0x406b			   // RV:  00H	TIMER1控制寄存器3
 #define T1AFL									  0x80							//
 ...
 #define TIM1_CR4						*(_IO  uint8 FT_XDATA *)0x406c			   // RV:  00H	TIMER1控制寄存器4
 #define T1CST2									  0x04							//				 rw-- 换相状态机配置2
 #define T1CST1									  0x02							//				 rw-- 换相状态机配置1
 #define T1CST0									  0x01							//				 rw-- 换相状态机配置0

 #define TIM1_IER						*(_IO  uint8 FT_XDATA *)0x406d			   // RV:  00H	TIMER1中断控制
 ...
 #define TIM1_SR						*(_IO  uint8 FT_XDATA *)0x406e			   // RV:  00H	TIMER1状态
 #define T1ADIF									  0x20							//
 ...
 #define TIM1_DBR1						*(_IO uint16 FT_XDATA *)0x4074			   // RV:0000H	TIMER1在CST=1的数据
 #define TIM1_DBR2						*(_IO uint16 FT_XDATA *)0x4076			   // RV:0000H	TIMER1在CST=2的数据
 #define TIM1_DBR3						*(_IO uint16 FT_XDATA *)0x4078			   // RV:0000H	TIMER1在CST=3的数据
 #define TIM1_DBR4						*(_IO uint16 FT_XDATA *)0x407a			   // RV:0000H	TIMER1在CST=4的数据
 #define TIM1_DBR5						*(_IO uint16 FT_XDATA *)0x407c			   // RV:0000H	TIMER1在CST=5的数据
 #define TIM1_DBR6						*(_IO uint16 FT_XDATA *)0x407e			   // RV:0000H	TIMER1在CST=6的数据
 #define TIM1_DBR7						*(_IO uint16 FT_XDATA *)0x4080			   // RV:0000H	TIMER1在CST=7的数据
 ...
 #define TIM1_BCOR						*(_IO uint16 FT_XDATA *)0x4070			   // RV:0000H	捕获基本定时器计数值滤波值
 #define TIM1__BCNTR					*(_IO uint16 FT_XDATA *)0x4082			   // RV:0000H	基本定时器计数值
 #define TIM1__BCCR						*(_IO uint16 FT_XDATA *)0x4084			   // RV:0000H	捕获基本定时器计数值
 #define TIM1__BARR						*(_IO uint16 FT_XDATA *)0x4086			   // RV:0000H	基本定时器自动重载值
 #define TIM1__RARR						*(_IO uint16 FT_XDATA *)0x4088			   // RV:0000H	重载定时器自动重载值
 #define TIM1__RCNTR					*(_IO uint16 FT_XDATA *)0x408a			   // RV:0000H	重载定时器计数值
 #define TIM1__ITRIP					*(_I  uint16 FT_XDATA *)0x4098			   // RV:0000H	滤波后的母线电流
 #define TIM1__UCOP						*(_IO uint16 FT_XDATA *)0x408c
 #define TIM1__UFLP						*(_IO uint16 FT_XDATA *)0x408e
 #define TIM1__URES						*(_IO uint16 FT_XDATA *)0x4090
 #define TIM1__UIGN						*(_IO uint16 FT_XDATA *)0x4092
 #define TIM1_KF						*(_IO uint16 FT_XDATA *)0x4094
 #define TIM1_KR						*(_IO uint16 FT_XDATA *)0x4096
/******************************************************************************///TIM2
...
/******************************************************************************///SYSTICK
 #define SYST_ARR						*(_IO uint16 FT_XDATA *)0x4064			   // RV:5dbfH	SysTick重载值
/******************************************************************************///DRV
...
SFR(DRV_OUT, 0xf8);
 ...
 #define DRV_DR							*(_IO uint16 FT_XDATA *)0x4058			   // RV:0000H	手动控制输出时设定的比较值
 #define DRV_COMR						*(_IO uint16 FT_XDATA *)0x405a			   // RV:0000H	计数器的比较匹配值

 #define DRV_CMR						*(_IO uint16 FT_XDATA *)0x405c			   // RV:0000H	输出配置寄存器
 ...
 #define DRV_ARR						*(_IO uint16 FT_XDATA *)0x405e			   // RV:0000H	计数器重载值
 #define DRV_DTR						*(_IO  uint8 FT_XDATA *)0x4060			   // RV:  00H	上下管死区时间
 #define DRV_CNTR						*(_IO uint16 FT_XDATA *)0x4067			   // RV:0000H	计数器值

 #define DRV_SR							*(_IO  uint8 FT_XDATA *)0x4061			   // RV:  00H	Driver状态寄存器
 ...
 #define DRV_CR							*(_IO  uint8 FT_XDATA *)0x4062			   // RV:  00H	Driver控制寄存器
...
#define	WDT_EN									  0x20							//				 rw-- WDT使能
 ...
 #define WDT_CR							*(_IO  uint8 FT_XDATA *)0x4026			   // RV:  00H	看门狗控制寄存器
 #define WDTF									  0x02							//				 rw0- 看门狗复位标志
 #define WDTRF									  0x01							//				 rw-- 看门狗初始化标志

 #define WDT_ARR						*(_IO  uint8 FT_XDATA *)0x4027			   // RV:  00H	看门狗重载计数器
/******************************************************************************///GPIO
...
// sbit	   GP45										= P4^5;
 ...
 SFR(P0_OE,	0xfc);
 #define P0_PU							*(_IO  uint8 FT_XDATA *)0x4053			   // RV:  00H	P0上拉使能寄存器
...
// sfr	   P1_OE						  =	0xfd;								  // RV:  00H  P1输出使能寄存器
 ...
 SFR(P1_IF,	0xd2);
 SFR(P1_OE,	0xfd);
 #define P1_PU							*(_IO  uint8 FT_XDATA *)0x4054			   // RV:  00H	P1上拉使能寄存器
 ...
 #define P1_AN							*(_IO  uint8 FT_XDATA *)0x4050			   // RV:  00H	P1模拟使能寄存器
 #define P17									  0x80							//				 rw-- 使能端口7
 #define P16									  0x40							//				 rw-- 使能端口6
 #define P15									  0x20							//				 rw-- 使能端口5
 #define P14									  0x10							//				 rw-- 使能端口4
...
// sfr	   P2_OE						  =	0xfe;								  // RV:  00H  P2输出使能寄存器
 ...
 SFR(P2_IF,	0xd4);
 SFR(P2_OE,	0xfe);
 #define P2_PU							*(_IO  uint8 FT_XDATA *)0x4055			   // RV:  00H	P2上拉使能寄存器
 #define P2_AN							*(_IO  uint8 FT_XDATA *)0x4051			   // RV:  00H	P2模拟使能寄存器
 ...
 SFR(P3_OE,	0xff);
 #define P3_PU							*(_IO  uint8 FT_XDATA *)0x4056			   // RV:  00H	P3上拉使能寄存器
 ...
 #define P3_AN							*(_IO  uint8 FT_XDATA *)0x4052			   // RV:  00H	P3模拟使能寄存器
 ...
 SFR(P4_OE,	0xe9);
 #define P4_PU							*(_IO  uint8 FT_XDATA *)0x4057			   // RV:  00H	P4上拉使能寄存器
 ...
 #define PH_SEL							*(_IO  uint8 FT_XDATA *)0x404c			   // RV:  00H	端口复用
 ...
 #define PH_SEL1						*(_IO  uint8 FT_XDATA *)0x404d			   // RV:  00H	端口复用
 #define SPICT									  0x04							//				 rw-- SPI功能转移
 #define T4CT									  0x02							//				 rw-- TIM4功能转移
 #define T3CT									  0x01							//				 rw-- TIM3功能转移

/******************************************************************************///ADC
 #define ADC_MASK						*(_IO uint16 FT_XDATA *)0x4036			   // RV:3000H	ADC通道使能和部分通道采样时间配置
 ...
 #define ADC_SYSC						*(_IO  uint8 FT_XDATA *)0x4038			   // RV:  33H	ADC通道采样时间配置

 #define ADC_CR							*(_IO  uint8 FT_XDATA *)0x4039			   // RV:  00H	ADC控制寄存器
 ...
 #define ADC0_DR						*(_I  uint16 FT_XDATA *)0x0300			   // RV:0000H	ADC通道0转换结果
 #define ADC1_DR						*(_I  uint16 FT_XDATA *)0x0302			   // RV:0000H	ADC通道1转换结果
 #define ADC2_DR						*(_I  uint16 FT_XDATA *)0x0304			   // RV:0000H	ADC通道2转换结果
 #define ADC3_DR						*(_I  uint16 FT_XDATA *)0x0306			   // RV:0000H	ADC通道3转换结果
 #define ADC4_DR						*(_I  uint16 FT_XDATA *)0x0308			   // RV:0000H	ADC通道4转换结果
 #define ADC5_DR						*(_I  uint16 FT_XDATA *)0x030a			   // RV:0000H	ADC通道5转换结果
 #define ADC6_DR						*(_I  uint16 FT_XDATA *)0x030c			   // RV:0000H	ADC通道6转换结果
 #define ADC7_DR						*(_I  uint16 FT_XDATA *)0x030e			   // RV:0000H	ADC通道7转换结果
 #define ADC8_DR						*(_I  uint16 FT_XDATA *)0x0310			   // RV:0000H	ADC通道8转换结果
 #define ADC9_DR						*(_I  uint16 FT_XDATA *)0x0312			   // RV:0000H	ADC通道9转换结果
 #define ADC10_DR						*(_I  uint16 FT_XDATA *)0x0314			   // RV:0000H	ADC通道10转换结果
 #define ADC11_DR						*(_I  uint16 FT_XDATA *)0x0316			   // RV:0000H	ADC通道11转换结果
 #define ADC12_DR						*(_I  uint16 FT_XDATA *)0x0318			   // RV:0000H	ADC通道12转换结果
 #define ADC13_DR						*(_I  uint16 FT_XDATA *)0x031a			   // RV:0000H	ADC通道13转换结果
 #define ADC14_DR						*(_I  uint16 FT_XDATA *)0x031c			   // RV:0000H	ADC通道14转换结果
/*****************************************************************************///DAC
 #define DAC_CR							*(_IO  uint8 FT_XDATA *)0x4035			   // RV:  00H	DAC控制寄存器
 ...
 #define DAC0_DR						 *(_IO	uint8 FT_XDATA *)0x404b				// RV:	00H	 DAC0输出值[8:1]
 #define DAC1_DR						 *(_IO	uint8 FT_XDATA *)0x404A				// RV:	00H	 DAC1输出值
 #define DAC0_DR_0								  0x80							 //	RV:	 00H  DAC0输出值[0]

/******************************************************************************///DMA
 #define DMA0_CR0						*(_IO  uint8 FT_XDATA *)0x403a			   // RV:  00H	DMA通道0配置 & DMA公共配置
 #define DMA1_CR0						*(_IO  uint8 FT_XDATA *)0x403b			   // RV:  00H	DMA通道1配置 & Debug设置
 ...
 #define DMA0_LEN						*(_IO  uint8 FT_XDATA *)0x403c			   // RV:  00H	DMA通道0传输长度配置
 #define DMA1_LEN						*(_IO  uint8 FT_XDATA *)0x403d			   // RV:  00H	DMA通道1传输长度配置

 #define DMA0_BA						*(_IO uint16 FT_XDATA *)0x403e			   // RV:0000H	DMA通道0传输地址配置
 #define DMA1_BA						*(_IO uint16 FT_XDATA *)0x4040			   // RV:0000H	DMA通道1传输地址配置
/******************************************************************************///VREF & VHALF
 #define VREF_VHALF_CR					*(_IO  uint8 FT_XDATA *)0x404f			   // RV:  00H	VREF & VHALF控制寄存器
...
/******************************************************************************///AMP
 #define AMP0_GAIN						*(_IO  uint8 FT_XDATA *)0x4034			   // RV:  00H	AMP0 PGA SET
 #define AMP0_GAIN2								  0x04							//				 rw-- AMP0放大倍数
 #define AMP0_GAIN1								  0x02							//				 rw-- AMP0放大倍数
 #define AMP0_GAIN0								  0x01							//				 rw-- AMP0放大倍数
 
 #define AMP_CR							*(_IO  uint8 FT_XDATA *)0x404e			   // RV:  00H	运放控制寄存器
...
/******************************************************************************///CMP
 ...
 #define CMP_SAMR						*(_IO  uint8 FT_XDATA *)0x40ad			   // RV:  00H	比较器采样配置
/******************************************************************************///FLASH
...
/******************************************************************************///CRC
 #define CRC_DIN						*(_O   uint8 FT_XDATA *)0x4021			   // RV:  00H	CRC数据输入寄存器

 #define CRC_CR							*(_IO  uint8 FT_XDATA *)0x4022			   // RV:	0H	CRC控制寄存器
 ...
 #define CRC_DR							*(_IO  uint8 FT_XDATA *)0x4023			   // RV:  00H	CRC结果输出寄存器
 #define CRC_BEG						*(_IO  uint8 FT_XDATA *)0x4024			   // RV:  00H	CRC自动计算的起始位置
 #define CRC_CNT						*(_IO  uint8 FT_XDATA *)0x4025			   // RV:  00H	CRC块数计数器
/******************************************************************************///TSD
 #define TSD_CR							*(_IO  uint8 FT_XDATA *)0x402f			   // RV:	0H	TSD控制寄存器
