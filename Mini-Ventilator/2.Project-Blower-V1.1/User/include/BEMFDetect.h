/* --------------------------- (C) COPYRIGHT 2021 Fortiortech ShenZhen -----------------------------
    ...
    Version        : V1.0
    Date           : 2021-12-30
    Description    : This file contains all common parameter used for Motor Control.
----------------------------------------------------------------------------------------------------
                                       All Rights Reserved
------------------------------------------------------------------------------------------------- */

/* Define to prevent recursive inclusion -------------------------------------------------------- */
#ifndef __B_E_M_F_DETECT_H_
#define __B_E_M_F_DETECT_H_
...
//定义使用BEMF启动最低转速,ROM
#define BEMFMotorStartSpeed             _Q15(1000.0 / MOTOR_SPEED_BASE)
#define BEMFMotorStartSpeedHigh         _Q15(30000.0 / MOTOR_SPEED_BASE)

//定义使用BEMF检测时间,ms
#define BEMF_START_DETECT_TIME          (100)//速度快的电机，时间可以缩短到50

//定义使用BEMF启动检测延时,ms
#define BEMF_START_DELAY_TIME           (10)//速度快的电机，时间可以缩短到15
...
typedef struct
{
    ...
    uint8  BEMF_Status[6];               //反电动势的状态，6拍的状态数组
    uint8  FRStatus;                    //正反转
...
extern uint8 CWCCWDetect(uint8 HallStatus, uint8 TargetFR);
