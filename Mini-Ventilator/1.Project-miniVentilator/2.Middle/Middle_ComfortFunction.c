...
uint16_t Mid_DelayIncreasePRESS(FlagStatus flag_delay,uint16_t real_time,uint8_t delay_min,uint16_t start_press,uint16_t point_press){
	...
	uint16_t up_press = 50;	//0.5cmH2O
	uint16_t point_count = 0; 
	uint16_t time_interval = 0;
	uint16_t timeinterval_count = 0;
	/* ERROR RUN. */
	if(flag_delay == RESET)
		return point_press;
	/* --------------------------------------------------------------------- */
	...
	/* Second stage. */
		...
		return_value = start_press + up_press;
	}
	/* Third stage. */
	else if(real_time < delaytime_s){
		point_count = (point_press - (start_press + up_press * 2)) / up_press;
		time_interval = (uint16_t)((delaytime_s * 0.8) / point_count);
		timeinterval_count = (uint16_t)((real_time - (delaytime_s * 0.2)) / time_interval);
		return_value = (start_press + up_press * 2) + (timeinterval_count * up_press);
	}
	/* Fourth stage -- Quit. */
	else{
...
/*!
	...
	@param[out]	 none
	@retval		 return_press:Run pressure.
*/
